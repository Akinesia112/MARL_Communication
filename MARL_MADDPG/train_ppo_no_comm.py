...
from baseline_no_comm import SimplePolicy, SimpleCritic
import os


class NoCommPPOTrainer:
    def __init__(self, scenario="Ant", agent_conf="2x4", lr=1e-4, gamma=0.99):
        ...
        self.policies = {agent: SimplePolicy(obs_dim, action_dim) for agent in self.agents}
        ...
        # checkpoints
        os.makedirs("checkpoints_ppo_nocomm", exist_ok=True)
    ...
    def save_checkpoint(self, suffix):
        ...
        path = os.path.join("checkpoints_ppo_nocomm", f"ppo_nocomm_{suffix}.pt")
    ...
    def train(self, n_iterations=1000, save_freq=50):
        ...
        for iteration in range(n_iterations):
            ...
            np.savez("ppo_nocomm_logs.npz", **self.logs)
        ...
        np.savez("ppo_nocomm_logs.npz", **self.logs)
        print("✓ Training logs saved to ppo_nocomm_logs.npz")
