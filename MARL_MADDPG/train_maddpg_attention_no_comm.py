...
class MADDPGAttentionTrainer:
    def __init__(
        ...
        self.checkpoint_dir = "checkpoints_maddpg_attention_no_comm"
        ...
        os.makedirs("checkpoints_maddpg", exist_ok=True)
    ...
    def save_actors(self, suffix):
        ...
        checkpoint = {agent: self.actors[agent].state_dict()
                      for agent in self.agents}
        path = os.path.join("checkpoints_maddpg_attention_no_comm", f"maddpg_attention_no_comm_{suffix}.pt")
    ...
    def train(self, n_episodes=1000, updates_per_step=1):
        ...
        for ep in range(n_episodes):
            ...
            np.savez("maddpg_attention_no_comm_logs.npz", **self.logs)
        ...
        np.savez("maddpg_attention_logs.npz", **self.logs)
        print("✓ Training logs saved to maddpg_attention_logs.npz")
