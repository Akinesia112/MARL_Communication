...
"""
Emergent-Communication MADDPG on MAMuJoCo Ant (4x2) - MLP version (NO attention)
...
class MADDPGCommTrainer:
    def __init__(
        ...
        # Save dir
        os.makedirs("checkpoints_maddpg_mlp_comm_ant_4x2", exist_ok=True)
    ...
    def save_actors(self, suffix):
        ...
        checkpoint = {agent: self.actors[agent].state_dict()
                      for agent in self.agents}
        path = os.path.join("checkpoints_maddpg_mlp_comm_ant_4x2", f"maddpg_comm_{suffix}.pt")
    ...
    def train(self, n_episodes=1000, updates_per_step=1):
        ...
        for ep in range(n_episodes):
            ...
            np.savez("maddpg_mlp_comm_logs_ant_4x2.npz", **self.logs)
        ...
        np.savez("maddpg_mlp_comm_logs_ant_4x2.npz", **self.logs)
        print("✓ Training logs saved to maddpg_mlp_comm_logs_ant_4x2.npz")
