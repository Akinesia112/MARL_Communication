...
class CommPPOTrainer:
    def __init__(
        ...
        # ----- networks: CommPolicyWithAttention + SimpleCritic -----
        self.policies = {
        ...
        # ----- checkpoints & logs -----
        self.ckpt_dir = "checkpoints_ppo_comm_ant_4x2"
        ...
        self.logs = {
            ...
            "policy_loss": [],
            "value_loss": [],
            "sigma": [],        # PPO 沒用到，為了畫圖留著
            "buffer_size": [],  # 每次 rollout 的長度
        }
    ...
    def collect_rollout(self, n_steps=2048):
        """
        Rollout 包含:
            obs, all_obs, actions, rewards, values, log_probs, dones
        all_obs 會用在 PPO update 時重新跑 comm policy。
        """
        rollout = {
            agent: {
                "obs": [],
        ...
        for step in range(n_steps):
            ...
            ).unsqueeze(0).to(self.device)  # [1, n_agents, obs_dim]
            ...
            for agent in self.agents:
                ...
                with torch.no_grad():
                    # CommPolicyWithAttention(obs, all_obs) -> (Normal dist, comm_info)
                    dist, _ = self.policies[agent](obs, all_obs_tensor)
                    value = self.critics[agent](obs)
                    action = dist.sample()
                    log_prob = dist.log_prob(action).sum(-1)

                rollout[agent]["obs"].append(obs_dict[agent])
                rollout[agent]["all_obs"].append(all_obs_tensor.cpu().numpy()[0])
    ...
    def train_step(self, rollout, clip_epsilon=0.2, epochs=4, batch_size=64):
        ...
        for epoch in range(epochs):
            for agent_idx, agent in enumerate(self.agents):
                ...
                all_obs = np.array(rollout[agent]["all_obs"], dtype=np.float32)  # [T, n_agents, obs_dim]
                ...
                for start in range(0, n_samples, batch_size):
                    ...
                    batch_all_obs = torch.FloatTensor(all_obs[batch_idx]).to(self.device)   # [B, n_agents, obs_dim]
                    ...
                    # policy update with comm (需要 all_obs)
    ...
    def save_checkpoint(self, suffix):
        ...
        path = os.path.join(self.ckpt_dir, f"ppo_comm_{suffix}.pt")
    ...
    def train(self, n_iterations=1000, save_freq=50):
        ...
        for iteration in range(n_iterations):
            ...
            np.savez("ppo_comm_logs_ant_4x2.npz", **self.logs)
        ...
        np.savez("ppo_comm_logs_ant_4x2.npz", **self.logs)
        print("✓ Training logs saved to ppo_comm_logs_ant_4x2.npz")
