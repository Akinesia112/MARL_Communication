...
class MAMuJoCoTrainer:
    def __init__(self, scenario="Ant", agent_conf="4x2", lr=3e-4, gamma=0.99):
        ...
        # 創建網絡
        self.policies = {
            agent: CommPolicyWithAttention(obs_dim,action_dim,hidden_dim=256, msg_len=4, # 頻寬控制
                        vocab_size=8,comm_tau=1.0,comm_hard=True,) 
        ...
        # --------- checkpoints & logs ---------
        self.ckpt_dir = "checkpoints_ppo_attention_no_comm_ant_4x2"
    ...
    def train_step(self, rollout, clip_epsilon=0.2, epochs=4, batch_size=64):
        ...
        # 存下這一個 train_step 的平均 loss，方便 print / log
    ...
    def save_checkpoint(self, iteration):
        ...
        out_path = os.path.join(
            self.ckpt_dir, f"model_iter_ppo_attention_no_comm_{iteration}.pt"
        )
        torch.save(checkpoint, out_path)
        print(f"  ✓ 模型已保存到 {out_path}")
    
    def train(self, n_iterations=1000, save_freq=50):
        """主訓練循環 - 存 reward / loss 到 npz"""
        ...
        for iteration in range(n_iterations):
            ...
            # 每個 iter 都覆寫一次 npz（方便中途看曲線）
            np.savez("ppo_attention_no_comm_logs_ant_4x2.npz", **self.logs)
        ...
        np.savez("ppo_attention_no_comm_logs_ant_4x2.npz", **self.logs)
        print("✓ Training logs saved to ppo_attention_no_comm_logs_ant_4x2.npz")
