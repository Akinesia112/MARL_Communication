#!/usr/bin/env python3
# -*- coding: utf-8 -*-

import os

# ---- MuJoCo / OpenGL: offscreen rendering ----
os.environ.setdefault("MUJOCO_GL", "egl")
os.environ.setdefault("PYOPENGL_PLATFORM", "egl")
os.environ.setdefault("MESA_GL_VERSION_OVERRIDE", "3.3")

import numpy as np
import torch
import imageio.v2 as imageio

from gymnasium_robotics import mamujoco_v1
from attention_emergent_comm import CommPolicyWithAttention


def make_env(
    scenario: str = "Ant",
    agent_conf: str = "4x2",
    render_mode: str = "rgb_array",
    max_episode_steps: int = 5000,
):
    """建立 MAMuJoCo Ant 環境（max_episode_steps 調大一點，比較不容易早死）"""
    env = mamujoco_v1.parallel_env(
        scenario=scenario,
        agent_conf=agent_conf,
        agent_obsk=1,
        render_mode=render_mode,
        max_episode_steps=max_episode_steps,
    )

    # 如果支援 camera config，就設定一個上帝視角
    if hasattr(env.unwrapped, "mujoco_renderer"):
        cam_cfg = {
            "distance": 8.0,
            "azimuth": 90.0,
            "elevation": -30.0,
            "lookat": [0, 0, 0.5],
        }
        try:
            env.unwrapped.mujoco_renderer.default_cam_config = cam_cfg
        except Exception:
            pass

    return env


def load_policies(env, checkpoint_path: str, device: torch.device):
    """建立每個 agent 的 CommPolicyWithAttention 並載入 checkpoint。"""
    agents = env.agents
    obs_dim = env.observation_space(agents[0]).shape[0]
    action_dim = env.action_space(agents[0]).shape[0]

    policies = {
        agent: CommPolicyWithAttention(
            obs_dim,
            action_dim,
            hidden_dim=256,
            msg_len=4,
            vocab_size=8,
            comm_tau=1.0,
            comm_hard=True,
        )
        for agent in agents
    }

    if checkpoint_path is not None and os.path.exists(checkpoint_path):
        ckpt = torch.load(checkpoint_path, map_location=device)
        for agent in agents:
            if agent not in ckpt:
                raise KeyError(
                    f"Checkpoint does not contain weights for agent '{agent}'. "
                    f"Available keys: {list(ckpt.keys())}"
                )
            policies[agent].load_state_dict(ckpt[agent])
        print(f"✓ Loaded checkpoint from {checkpoint_path}")
        model_name = os.path.splitext(os.path.basename(checkpoint_path))[0]
    else:
        print(
            f"⚠ Checkpoint not found at {checkpoint_path}, using random policies."
        )
        model_name = "ppo_attention_no_comm_random"

    for agent in agents:
        policies[agent].to(device)
        policies[agent].eval()

    return policies, model_name


@torch.no_grad()
def run_episode_and_record(
    env,
    policies,
    device: torch.device,
    model_name: str,
    episode_idx: int = 0,
    max_episode_steps: int = 1000,
    fps: int = 30,
):
    """
    單一 episode 錄影（比較短，方便 debug 用）。
    """
    obs_dict, _ = env.reset()
    agents = env.agents

    done = False
    step = 0
    episode_reward = {agent: 0.0 for agent in agents}
    frames = []

    while not done and step < max_episode_steps:
        # 準備 all_obs 給 communication 用
        all_obs_np = np.array([obs_dict[a] for a in agents], dtype=np.float32)
        all_obs = torch.from_numpy(all_obs_np).unsqueeze(0).to(device)  # [1, n_agents, obs_dim]

        actions_dict = {}
        for agent in agents:
            obs = torch.from_numpy(obs_dict[agent]).float().unsqueeze(0).to(device)  # [1, obs_dim]
            dist, _ = policies[agent](obs, all_obs)

            # eval 時採用 deterministic policy：使用 mean
            action = dist.mean
            actions_dict[agent] = action.cpu().numpy()[0]

        next_obs, rewards, terms, truncs, infos = env.step(actions_dict)

        for agent in agents:
            episode_reward[agent] += float(rewards[agent])

        frame = env.render()
        if frame is not None:
            frames.append(frame)

        step += 1
        done = any(terms.values()) or any(truncs.values())
        if not done:
            obs_dict = next_obs

    avg_rew = sum(episode_reward.values()) / len(agents)
    print(f"[Episode {episode_idx}] steps={step}, avg_rew={avg_rew:.2f}")
    print("  reward per agent:", episode_reward)

    # 存成 mp4
    if len(frames) > 0:
        os.makedirs("videos", exist_ok=True)
        out_path = os.path.join("videos", f"{model_name}_ep{episode_idx}_ant_4x2.mp4")
        imageio.mimsave(out_path, frames, fps=fps)
        print(f"✓ Saved video to {out_path}")
    else:
        print("⚠ No frames captured (env.render() returned None).")


@torch.no_grad()
def record_fixed_duration(
    env,
    policies,
    device: torch.device,
    model_name: str,
    total_seconds: int = 300,
    fps: int = 30,
    max_episode_steps: int = 5000,
):
    """
    把多個 episode 串成一支固定時長（例如 5 分鐘）的影片。

    - total_seconds: 影片長度（秒），預設 300 s = 5 min
    - fps:           frame rate
    - max_episode_steps: 單一 episode 的最多步數（避免卡死）
    """
    os.makedirs("videos", exist_ok=True)
    out_path = os.path.join("videos", f"{model_name}_ant_4x2.mp4")

    obs_dict, _ = env.reset()
    agents = env.agents

    target_frames = int(total_seconds * fps)
    frame_count = 0
    episode_idx = 0
    step_in_ep = 0
    episode_reward = {agent: 0.0 for agent in agents}

    print(
        f"▶ Start recording fixed duration: {total_seconds}s, "
        f"target_frames={target_frames}, fps={fps}"
    )

    with imageio.get_writer(out_path, fps=fps) as writer:
        while frame_count < target_frames:
            all_obs_np = np.array([obs_dict[a] for a in agents], dtype=np.float32)
            all_obs = torch.from_numpy(all_obs_np).unsqueeze(0).to(device)

            actions_dict = {}
            for agent in agents:
                obs = torch.from_numpy(obs_dict[agent]).float().unsqueeze(0).to(device)
                dist, _ = policies[agent](obs, all_obs)
                action = dist.mean  # deterministic
                actions_dict[agent] = action.cpu().numpy()[0]

            next_obs, rewards, terms, truncs, infos = env.step(actions_dict)

            for agent in agents:
                episode_reward[agent] += float(rewards[agent])

            frame = env.render()
            if frame is not None:
                writer.append_data(frame)
                frame_count += 1

            step_in_ep += 1
            done = any(terms.values()) or any(truncs.values()) or (
                step_in_ep >= max_episode_steps
            )

            if done:
                avg_rew = sum(episode_reward.values()) / len(agents)
                print(
                    f"  Episode {episode_idx}: steps={step_in_ep}, "
                    f"avg_rew={avg_rew:.2f}"
                )
                # reset 下一個 episode
                obs_dict, _ = env.reset()
                agents = env.agents
                episode_idx += 1
                step_in_ep = 0
                episode_reward = {agent: 0.0 for agent in agents}
            else:
                obs_dict = next_obs

    print(f"✓ Saved fixed-duration video to {out_path}")


def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # 跟訓練命名對齊：
    #   checkpoints_ppo_attention_no_comm/model_iter_ppo_attention_no_comm_best.pt
    checkpoint_path = os.path.join(
        "checkpoints_ppo_attention_no_comm_ant_4x2",
        "model_iter_ppo_attention_no_comm_best.pt",
    )

    env = make_env(
        scenario="Ant",
        agent_conf="4x2",
        render_mode="rgb_array",
        max_episode_steps=5000,
    )
    policies, model_name = load_policies(env, checkpoint_path, device)

    # （可選）先錄一個單 episode 檢查一下是不是正常
    # run_episode_and_record(
    #     env,
    #     policies,
    #     device,
    #     model_name,
    #     episode_idx=1,
    #     max_episode_steps=1000,
    #     fps=30,
    # )

    # 錄一支固定長度影片（預設 5 分鐘）
    record_fixed_duration(
        env,
        policies,
        device,
        model_name,
        total_seconds=300,  # 5 min
        fps=30,
        max_episode_steps=5000,
    )

    env.close()


if __name__ == "__main__":
    main()
