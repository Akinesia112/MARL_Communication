...
def load_policies(env, model_path, device):
    ...
    policies = {
        agent: GATPolicyNoComm(obs_dim, action_dim, hidden_dim=256).to(device)
        for agent in agents
    }
    ...
    print(f"✓ Loaded PPO No-Comm model from {model_path}")
    return policies
...
def evaluate_ppo_nocomm(
    model_path="checkpoints_ppo_gat_no_comm_ant_4x2/ppo_nocomm_best.pt",
    out_dir="videos",
    out_name="ppo_gat_no_comm_ant_4x2.mp4",
    mode="fixed_time",
