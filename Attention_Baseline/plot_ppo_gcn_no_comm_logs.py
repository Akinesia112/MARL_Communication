...
def main(
    log_path="ppo_gcn_no_comm_logs_ant_4x2.npz",
    out_dir="png/ppo_gcn_no_comm_ant_4x2",
):
    ...
    plt.ylabel("Reward")
    plt.title("PPO-GCN No-Comm: Reward Curve")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    reward_path = os.path.join(out_dir, "ppo_gcn_nocomm_reward.png")
    ...
    plt.xlabel("Iteration")
    plt.ylabel("Loss")
    plt.title("PPO-GCN No-Comm: Loss Curve")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    loss_path = os.path.join(out_dir, "ppo_gcn_nocomm_loss.png")
    ...
    ax2.set_ylabel("Buffer size (rollout length)", color="C1")
    ax2.tick_params(axis="y", labelcolor="C1")

    plt.title("PPO-GCN No-Comm: Sigma & Buffer (dummy)")
    fig.tight_layout()
    sigma_path = os.path.join(out_dir, "ppo_gcn_nocomm_sigma_buffer.png")
