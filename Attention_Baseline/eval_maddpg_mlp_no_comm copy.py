...
from baseline_no_comm import SimplePolicy  # your MLP Gaussian policy
...
def load_policies(env, checkpoint_path, device):
    ...
    if checkpoint_path is not None:
        ...
        print("⚠ Using untrained random MLP policies.")
        model_name = "random_mlp_no_comm"
...
def run_episode_and_record(env, policies, device, model_name, episode_idx=0,
    ...
    while not done and step < max_episode_steps:
        ...
        for agent in agents:
            ...
            # SimplePolicy returns a Normal distribution
    ...
    if len(frames) > 0:
        ...
        out_path = os.path.join("videos", f"{model_name}_ep{episode_idx}_mlp_no_comm_ant_4x2.mp4")
...
def record_fixed_duration(env, policies, device, model_name,
    ...
    os.makedirs("videos", exist_ok=True)
    out_path = os.path.join("videos", f"{model_name}_mlp_no_comm_ant_4x2.mp4")
...
def main():
    ...
    checkpoint_path = "checkpoints_maddpg_mlp_no_comm_ant_4x2/maddpg_nocomm_best.pt"
