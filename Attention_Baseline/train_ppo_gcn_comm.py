...
from gnn_policies import GCNPolicyComm



class CommPPOTrainer:
    def __init__(
        self,
        scenario="Ant",
        agent_conf="4x2",
        ...
        # ----- networks: CCNCommPolicy + SimpleCritic -----
        self.policies = {
            agent: GCNPolicyComm(
                obs_dim=obs_dim,
        ...
        self.critics = {
            agent: SimpleCritic(obs_dim).to(self.device)
            for agent in self.agents
        }
        ...
        # ----- checkpoints & logs -----
        self.ckpt_dir = "checkpoints_ppo_gcn_comm_ant_4x2"
    ...
    def collect_rollout(self, n_steps=2048):
        ...
        for step in range(n_steps):
            # all agents' obs for comm/attention
            all_obs_np = np.array([obs_dict[a] for a in self.agents], dtype=np.float32)
            all_obs_tensor = torch.FloatTensor(all_obs_np).unsqueeze(0).to(self.device)  # [1, N, obs_dim]
            ...
            for idx, agent in enumerate(self.agents):
                ...
                with torch.no_grad():
                    # CCNCommPolicy(obs, all_obs, self_index)
                    dist, _ = self.policies[agent](obs, all_obs_tensor, self_index=idx)
                    value = self.critics[agent](obs)
                    action = dist.sample()
                    log_prob = dist.log_prob(action).sum(-1)

                rollout[agent]["obs"].append(obs_dict[agent])
                rollout[agent]["all_obs"].append(all_obs_np)          # 存 [N, obs_dim]
    ...
    def train_step(self, rollout, clip_epsilon=0.2, epochs=4, batch_size=64):
        ...
        for epoch in range(epochs):
            for agent_idx, agent in enumerate(self.agents):
                obs = np.array(rollout[agent]["obs"], dtype=np.float32)          # [T, obs_dim]
                all_obs = np.array(rollout[agent]["all_obs"], dtype=np.float32)  # [T, N, obs_dim]
                ...
                for start in range(0, n_samples, batch_size):
                    ...
                    batch_obs = torch.FloatTensor(obs[batch_idx]).to(self.device)           # [B, obs_dim]
                    batch_all_obs = torch.FloatTensor(all_obs[batch_idx]).to(self.device)   # [B, N, obs_dim]
                    ...
                    # policy update with comm (需要 all_obs + self_index)
                    dist, _ = self.policies[agent](batch_obs, batch_all_obs, self_index=agent_idx)
    ...
    def save_checkpoint(self, suffix):
        ...
        path = os.path.join(self.ckpt_dir, f"ppo_gcn_comm_{suffix}.pt")
        torch.save(checkpoint, path)
        print(f"✓ Comm PPO saved: {path}")
    ...
    def train(self, n_iterations=1000, save_freq=50):
        ...
        for iteration in range(n_iterations):
            ...
            # PPO 沒有 sigma / buffer，為了和 MADDPG plot 相容，塞 dummy
            self.logs["sigma"].append(0.0)
            self.logs["buffer_size"].append(len(rollout[self.agents[0]]["rewards"]))

            np.savez("ppo_gcn_comm_logs_ant_4x2.npz", **self.logs)
        ...
        np.savez("ppo_gcn_comm_logs_ant_4x2.npz", **self.logs)
        print("✓ Training logs saved to ppo_gcn_comm_logs_ant_4x2.npz")


if __name__ == "__main__":
    trainer = CommPPOTrainer(
        scenario="Ant",
        agent_conf="4x2",
        lr=1e-4,
        gamma=0.99,
