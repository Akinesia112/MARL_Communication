...
# 對應 MLP baseline 的 log 檔名
LOG_PATH = "maddpg_mlp_no_comm_logs_ant_4x2.npz"
OUT_DIR = os.path.join("png", "maddpg_mlp_no_comm_ant_4x2")
...
def main():
    ...
    plt.xlabel("Episode")
    plt.ylabel("Reward")
    plt.title("MADDPG MLP No Comm: Reward vs Episode")
    plt.legend()
    ...
    plt.xlabel("Episode")
    plt.ylabel("Loss")
    plt.title("MADDPG MLP No Comm: Loss vs Episode")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    plt.title("MADDPG MLP No Comm: Exploration & Buffer Size")
    fig.tight_layout()
